...
export const AiStudioPlayground: React.FC<AiStudioPlaygroundProps> = ({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome-msg',
      role: 'assistant',
      content: 'Olá! Sou o seu assistente de estudos no AI Studio Playground. Como posso te ajudar hoje? Você pode enviar dúvidas, pedir resumos, colar textos ou anexar documentos e imagens.',
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se o usuário configurou uma chave de API própria, tenta chamada direta à API oficial do Google Gemini
      ...
      if (cleanApiKey) {
        try {
          ...
          if (directResponse.ok) {
            ...
            throw new Error(
              errData.error?.message || `Erro da API Gemini: Status ${directResponse.status}`
            );
          }
        } catch (directErr: any) {
          // Se falhou e existe servidor local, tenta proxy local como fallback
          console.warn('Falha na chamada direta da API Gemini, tentando backend local...', directErr);
          throw directErr;
        }
      } else {
        // Sem chave informada no cliente: tenta o endpoint proxy do backend `/api/gemini/chat`
        ...
        if (!response.ok) {
          ...
          try {
            ...
            if (response.status === 404) {
              msg = 'Servidor local não encontrado ou sem rota /api/gemini/chat. Configure sua chave de API nas Configurações do Playground.';
            }
          }
          throw new Error(msg);
        }
